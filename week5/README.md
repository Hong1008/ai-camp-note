...
### 🔴 Theme 12: 앙상블 학습 및 머신러닝 프로세스 마무리 (2026-04-30) [COMPLETED]
머신러닝의 꽃인 앙상블 기법을 이해하고, 실무에서 모델을 배포하기 전 최종 검증 프로세스와 고급 튜닝을 학습했습니다.
- **불균형 데이터 최적화**: `BalancedRandomForest` 도입 및 임계값(Threshold) 조정을 통해 비즈니스 가치(재현율)를 최적화.
- **피처 엔지니어링과 데이터 누수**: 정답지(Target)가 X에 포함되는 데이터 누수(Data Leakage) 현상 해결 및 트리 모델에서의 피처 엔지니어링의 역설(정보 손실) 확인.
- **정보의 시너지 (조합 변수)**: 비행 거리와 좌석 등급을 결합한 `class_dist_combined` 등 모델에게 지름길을 제공하는 고급 피처 엔지니어링 실습.
- **초정밀 하이퍼파라미터 튜닝 (Optuna)**: Grid/Random Search를 넘어 베이지안 기반 최적화 프레임워크인 `Optuna`를 활용한 효율적인 모델 튜닝.
...
- [04-30.ipynb](04-30.ipynb): 앙상블 학습(Random Forest) 및 파이프라인 실습.
- **트러블슈팅 리포트**:
    ...
    - [2026-04-30 리포트](./troubleshooting/2026-04-30.md): 앙상블 모델 과적합 제어 및 파이프라인 구축 트러블슈팅.
